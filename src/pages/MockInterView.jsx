...
const MockInterview = () => {
  ...
  useEffect(() => {
    const fetchQuestions = async () => {
      if (!resumeText || !jobRole || !difficulty) {
        console.log(resumeText,jobRole,difficulty)
        navigate('/');
        return;
      }
      ...
      try {
        ...
        const response = await fetch("https://airesumeproapi.onrender.com/api/mockinterview", {
          method: "POST",
          headers: {
  ...
  const evaluateAnswers = async () => {
    ...
    try {
      ...
      const response = await fetch("https://airesumeproapi.onrender.com/api/evaluate-answers", {
        method: "POST",
        headers: {
          "Content-Type": "application/json",
          "Authorization": `Bearer ${token}`
    ...
    return (
      <div className="h-screen">
        ...
        <div className="p-6 max-w-6xl m-auto">
          <h1 className="text-3xl font-bold text-center mb-8">Interview Results</h1>
          
          <div className="grid grid-cols-3 gap-4 mb-8">
            <div className="bg-green-100 p-4 rounded-lg text-center">
              <h3 className="text-xl font-semibold">Correct Answers</h3>
              <p className="text-4xl font-bold text-green-600">{evaluationResults.correctCount}</p>
            </div>
            <div className="bg-red-100 p-4 rounded-lg text-center">
              <h3 className="text-xl font-semibold">Wrong Answers</h3>
              <p className="text-4xl font-bold text-red-600">{evaluationResults.wrongCount}</p>
            </div>
            <div className="bg-yellow-100 p-4 rounded-lg text-center">
              <h3 className="text-xl font-semibold">Skipped Questions</h3>
              <p className="text-4xl font-bold text-yellow-600">{skippedCount}</p>
            </div>
          </div>
          ...
          <div className="space-y-6">
            {questions.map((question, index) => (
              <div key={index} className="border border-gray-200 rounded-lg p-4">
                <h3 className="text-xl font-semibold mb-2">Q{index + 1}: {question}</h3>
                <p className="mb-2"><strong>Your Answer:</strong> {answers[index] || "Not answered"}</p>
                <p className="mb-2"><strong>Expected Answer:</strong> {expectedAnswers[index]}</p>
                <div className="p-3 rounded" style={{ 
                  backgroundColor: evaluationResults.evaluation[index]?.includes("Correct") ? '#dcfce7' : '#fee2e2'
                }}>
                  <p className={evaluationResults.evaluation[index]?.includes("Correct") ? "text-green-700" : "text-red-700"}>
                    <strong>{evaluationResults.evaluation[index]?.includes("Correct") ? "✓ Correct" : "✗ Wrong"}</strong>
                  </p>
            ...
            <div className="my-4 shadow-xl p-4 rounded-2xl">
              <div className="flex justify-between items-center mb-4">
                ...
                <Button
                  ...
                  onClick={() => {
                    setAnswers(prev => ({ ...prev, [currentQuestionIndex]: "Skipped" }));
